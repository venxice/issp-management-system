...
<form id="mainForm" action="<?= site_url('employee/proposed-ict-strategy/enterprise-architecture/save') ?>" method="post" enctype="multipart/form-data">
    <?= csrf_field() ?>
...
function finalizeSave(formDataObj, showAlert) {
    ...
    try {
        ...
        const verify = localStorage.getItem('enterprise-architecture-form');
...
function restoreFilePreview(input, dataUrl) {
    ...
    removeBtn.addEventListener('click', function(e) {
        ...
        try {
            const savedData = JSON.parse(localStorage.getItem('enterprise-architecture-form') || '{}');
            delete savedData[input.name];
            localStorage.setItem('enterprise-architecture-form', JSON.stringify(savedData));
            if (typeof updateStatusIndicators === 'function') updateStatusIndicators();
